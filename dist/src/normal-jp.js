...
export function kanaToHiragana(input) {
    let result = '';
...
export function countMora(text) {
    return [...text].filter((character, index) => index === 0 || !SKIP_FOR_MORA_COUNT.includes(character.codePointAt(0))).length;
}
export function moraSubstring(input, startIndex, endIndex) {
